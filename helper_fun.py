...
def ValidateSanitizePath(path: str) -> Union[str, bool]:
    """Takes a path and performs steps:
    - removes any spaces from left and right side of string path
    - removes any single or double quotes from left/right side
    - checks if the path exists or not    

    Args:
        path (str): Path as string to be validated/sanitized(leading/trailing)

    Returns:
        str: validated/sanitized string path 
        or
        bool: if string passed in fails any of tests return false
    """
...
#        pathExists = checkIfPathExists(path)    
        print(f"The value of path is {path}, and length is {len(path)}")
        return path
#        if pathExists:
#            return path
#        else:
...
def CheckOnMkDir(DirToCreate: str) -> bool:
    ...
    DirToCreate = ValidateSanitizePath(DirToCreate)
    ...
    if not checkIfPathExists(DirToCreate):
        try: 
            ...
            print(f"Folder {DirToCreate} created")
            return True
